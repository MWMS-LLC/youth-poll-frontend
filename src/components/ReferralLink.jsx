...
const ReferralLink = ({ userUUID }) => {
  ...
  // Bouncing animation every 10 seconds
  useEffect(() => {
    const bounceInterval = setInterval(() => {
      setIsBouncing(true);
      setTimeout(() => setIsBouncing(false), 1000); // Bounce for 1 second
  ...
  return (
    <div className={`bg-gray-900/30 backdrop-blur-sm rounded-lg p-4 mb-4 border border-gray-600/40 transition-transform duration-300 ${
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              onClick={() => {
                const text = `Check out this teen poll app! ${referralLink}`;
                const url = `https://www.instagram.com/myworldmysay`;
                window.open(url, '_blank');
              }}
              className={`px-3 py-2 rounded text-sm font-medium transition-colors ${
                hasConnectedAccounts 
            ...
            <button
              ...
              className={`px-3 py-2 rounded text-sm font-medium transition-colors ${
                hasConnectedAccounts 
                  ? 'bg-yellow-600/40 hover:bg-yellow-500/50 text-white border border-yellow-400/60' 
                  : 'bg-yellow-600/20 hover:bg-yellow-500/30 text-white/70 border border-dashed border-yellow-400/40'
              }`}
            >
              Snapchat
            </button>
            ...
            <button
              onClick={() => {
                const text = `Check out this teen poll app! ${referralLink}`;
                // TikTok: Just open TikTok main page since direct sharing is limited
                window.open('https://www.tiktok.com', '_blank');
                // Also try to copy to clipboard as a bonus
                try {
                  navigator.clipboard.writeText(text);
                  console.log('Sharing text copied to clipboard for TikTok');
                } catch (error) {
